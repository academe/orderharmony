 ...
 * TODO: tests
 * TODO: think about how different API versions would work - perhaps an injected class or factory?
 * TODO: how to get data back as an array rather than nested objects?
 * TODO: error handling (on connection and on result from server).
 * TODO: parsing of return data in a sensible according to the entities being passed around.
...
class OrderHarmony
{
    ...
    {
        $uri = $this->getBaseUri('/customer/' . $customerId . '/orders');
        $AuthQuery = $this->uriAuthQuery($uri);
        return $this->rest->get($this->GetServerUrl($uri), $AuthQuery, $this->restHeaders, $this->restOptions);
    }
    ...
    {
        $strProductsData = json_encode($productsData);

        $uri = $this->getBaseUri('/products/save');
        $AuthQuery = $this->uriAuthQuery($uri);
        $AuthQuery = array_merge($AuthQuery, array('products' => $strProductsData));
        return $this->rest->put($this->GetServerUrl($uri), $AuthQuery, $this->restHeaders, $this->restOptions);
    }
}
